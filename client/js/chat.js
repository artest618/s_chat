$(document).ready(function() {
    ...
    var from = '', to = 'all'; //设置默认接收对象为"所有人"
    xhrGet('/getSignedUser', {}, function(data){
        from = data.name;
        //发送用户上线信号
...
function xhrGet(url, params, callback){
    $.ajax({
        ...
        beforeSend: function(x, settings) {
            // Common.maskBox.show();
//            if((Common.Ajax.proccessing <= 1 || $("#maskbox").length < 1) && Common.Ajax.loadingMask){
//                Common.maskBox.show();
//            }
        },
...
//                Common.showAlert('当前访问人数过多，请稍候再试！', ['确定']);
//            }
            // Common.maskBox.hide();
//            setTimeout(function(){
